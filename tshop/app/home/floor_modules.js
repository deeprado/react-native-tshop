    ...
    return (
      <View style={{alignItems: 'center'}}>
        ...
        {(i.tplId === 1 ||
          ...
          i.tplId === 11 ||
          i.tplId ===
            12) /*通栏1个1.1，1.2，平均两个，平均三个,这里的10是710*220，记得换！！！*/ && (
          <View
            style={[
              floorStyles.module,
              {
        ...
        {(i.tplId === 4 ||
          i.tplId === 3) /*4:左边468，右边226  3:左边226，右边468*/ && (
          <View
            style={[
              floorStyles.module,
              {
                ...
                <TouchableOpacity
                  ...
                  <View
                    ...
                    <Image
                      ...
                      style={[
                        {
                          width: px(220),
                          height: i.tplId === 14 ? px(280) : px(220),
                        },
                        floorStyles.moduleImage,
                      ]}
                      source={{uri: k.imageUrl}}
                  ...
                  {i.tplId === 15 && k.urlType === 'sku' && (
                    <View style={{width: px(220), alignItems: 'center'}}>
                      <Text
                        style={floorStyles.moduleGoodName}
                        allowFontScaling={false}>
                        {k.title}
                      </Text>
                      <View style={floorStyles.modulePrice}>
                        <Text
                          style={floorStyles.moduleGoodPrice}
                          allowFontScaling={false}>
                          ￥{toInt(k.salePrice)}
                        </Text>
                        <Text
                          style={floorStyles.moduleGoodMarket}
                          allowFontScaling={false}>
                          ￥{toInt(k.marketPrice)}
                        </Text>
                      </View>
        ...
        {i.tplId === 8 && (
          <View style={[floorStyles.module]}>
            <View style={{marginRight: px(16)}}>
              <TouchableWithoutFeedback
                onPress={() =>
                  this.props.goOtherPage(i.moduleTemplateImages[0])
                }>
                <View
                  style={[
                    floorStyles.moduleImage_,
                    {width: px(347), height: px(496)},
                  ]}>
                  <Image
                    resizeMode="cover"
                    resizeMethod="scale"
            ...
            <View
              style={{flexDirection: 'row', width: px(347), flexWrap: 'wrap'}}>
              {i.moduleTemplateImages.map((k, k_index) => {
                return k_index > 0 ? (
                  <TouchableWithoutFeedback
                    key={k.imgId}
        ...
        {i.tplId == 6 && (
          <View style={[floorStyles.module]}>
              ...
              <TouchableWithoutFeedback
                ...
                <View
                  style={[
                    floorStyles.moduleImage_,
                    {
                      width: i.tplId === 8 ? px(347) : px(258),
                      height: i.tplId === 8 ? px(496) : px(476),
                    },
                  ]}>
                  <Image
                    resizeMode="cover"
            ...
            <View
              style={{
                /*flexDirection: 'row',*/ width: px(436),
                flexWrap: 'wrap',
              }}>
              <View style={{flexDirection: 'row', width: px(436)}}>
                {i.moduleTemplateImages.map((k, k_index) => {
                  return k_index > 0 && k_index < 3 ? (
                    <TouchableWithoutFeedback
                      key={k.imgId}
                      onPress={() => this.props.goOtherPage(k)}>
                      <View
...
exports.Module = class extends React.Component {
  render() {
    ...
    return (
      <View onLayout={e => this.onLayout(e.nativeEvent)}>
        <View
          style={[
            floorStyles.floor,
            {
              marginBottom: index === 0 && item.tplId === 13 ? px(0) : px(20),
              paddingBottom: index === 0 && item.tplId === 13 ? px(0) : px(20),
            },
          ]}>
          {(item.isApplName === 1 ||
            ...
            <View
              style={[
                floorStyles.floorTitle,
                {paddingBottom: item.isApplName === 0 ? px(0) : px(24)},
              ]}>
              {item.isApplName === 0 && item.titleImg && (
                <Image
                  source={{uri: item.titleImg}}
                  style={{
                    width: px(item.titleImgWidth),
                    height: px(item.titleImgHeight),
                  }}
                />
              )}
              {item.isApplName === 1 && (
                <Text
                  allowFontScaling={false}
                  style={floorStyles.floorTitleTxt}>
                  {item.moduleName}
                </Text>
              )}
            </View>
          ...
          <View
            style={{
              paddingTop:
                item.isApplName === 0 && !item.titleImg ? px(24) : px(0),
            }}>
            {item.moduleTemplates &&
              item.moduleTemplates.map((i, _index) => (
                <ModuleItem
