    ...
    return (
      <View style={{flex: 1}}>
        {this.state.banners.length === 1 && (
          <TouchableWithoutFeedback
            ...
            <Image
              ...
              source={{uri: this.state.banners[0].showImg}}
              resizeMode="contain"
              resizeMethod="scale"
            />
          </TouchableWithoutFeedback>
        ...
        {this.state.quicks.length > 0 && (
          <View style={bannerStyle.box}>
            <ImageBackground
              resizeMode="cover"
              resizeMethod="scale"
              style={bannerStyle.container}
              source={{uri: require('../images/shop-quick-bottom')}}>
              {this.state.quicks.map((item, index) => (
                <TouchableWithoutFeedback
        ...
        <View style={goodStyles.sGoodL}>
          <View style={goodStyles.sGoodImage}>
            ...
            {item.limitStock === 0 && (
              <View style={goodStyles.goods_img_cover}>
                <Text allowFontScaling={false} style={goodStyles.goods_img_txt}>
                  抢光了
                </Text>
              </View>
            )}
              ...
              {item.labelList &&
                ...
                item.labelList.map(label => (
                  <Image
                    ...
                    style={[
                      goodStyles.labelImg,
                      {width: px(label.width), height: px(label.height)},
                    ]}
          ...
          <View style={goodStyles.sessionName}>
            ...
            <View style={goodStyles.goodsShowDesc_}>
              {(item.isInBond == 1 || item.isForeignSupply == 2) && (
                <View
                  style={[
                    goodStyles.flag_,
                    item.isInBond == 1 ? goodStyles.flagB : goodStyles.flagZ,
                  ]}>
                  <Text style={goodStyles.flagTxt} allowFontScaling={false}>
                    {item.isInBond == 1
                      ? '保税'
              ...
              <Text
                style={goodStyles.goodsShowDesc}
                allowFontScaling={false}
                numberOfLines={2}>
                {(item.isInBond == 1 || item.isForeignSupply == 2) && (
                  <Text style={goodStyles.flag} allowFontScaling={false}>
                    {item.isInBond == 1 ? '保税' : '直邮'}{' '}
                  </Text>
                )}
                {item.goodsShowDesc}
              </Text>
            ...
            <View style={goodStyles.sessionNoName}>
              <View
                style={[
                  goodStyles.sessionPrice,
                  {flexDirection: 'column', alignItems: 'flex-start'},
                ]}>
                <Text allowFontScaling={false} style={goodStyles.salePrice}>
                  ￥
                  <Text allowFontScaling={false} style={goodStyles.salePrice_}>
                    {util_cools.spliceNum(item.salePrice)[0]}
                  </Text>
                  .{util_cools.spliceNum(item.salePrice)[1]}
                </Text>
                <Text
                  allowFontScaling={false}
    ...
    return (
      <TouchableOpacity
        ...
        <View style={goodStyles.bGood}>
          <View
            ...
            <View style={goodStyles.imageBox}>
              <Image
                resizeMethod="scale"
                source={{
                  uri: this.props.show ? item.image : require('../images/img2'),
                }}
          ...
          <View
            style={[
              goodStyles.sessionNoName,
              goodStyles.sessionNoNameBig,
              {alignItems: 'center'},
            ]}>
            <View
              style={[
    ...
    if (item.data2 && item.data2.id) {
      return (
        <View
          ...
          style={[goodStyles.goodsBox1, goodStyles.goodsBox3]}>
    ...
    return (
      <View style={styleSearchBar.header}>
        <TouchableOpacity activeOpacity={0.9} onPress={() => this.shareTo()}>
          <View style={styleSearchBar.back}>
            <Image
              source={{
                uri:
                  'http://img.cdn.daling.com/data/files/mobile/img/dalingjia.jpg',
              }}
              style={styleSearchBar.shopLogo}
            />
            <Image
              style={styleSearchBar.headerShareImg}
        ...
        <TouchableWithoutFeedback onPress={() => this.goSearch()}>
          <View
            style={[
              styleSearchBar.headerSearchBar,
              {
                backgroundColor: '#fff',
              },
            ]}>
            <Image
              style={styleSearchBar.headerSearchImg}
              source={{uri: require('../images/icon-search-gray')}}
            />
            <Text
              allowFontScaling={false}
              style={styleSearchBar.headerSearchInput}>
              在
...
export default class extends React.Component {
  ...
  async loadBanner() {
    try {
      let res = await request.get(
        `/banner/findBannerAndQuickList.do?categoryId=`,
      );
      res.type = 'banner';
      res.index = 'banner';
      res.tt = Date.now();
  ...
  async getModules() {
    ...
    let list = [];
    try {
      let moduleList = await request.get(
        `/module/findModuleListV2.do?categoryId=`,
      );
  ...
  async getNextProducts() {
    ...
    try {
      let res = await request.get(
        `/goods/list.do?limit=20&start=${this.start}&categoryId=`,
      );
