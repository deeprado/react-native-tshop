      ...
      <View style={styles.container}>
        ...
        <View style={[styles.input, {marginBottom: px(62)}]}>
          ...
          <Text
            ...
            style={this.state.sent ? styles.sent : styles.send}
            onPress={() => this.sendCode()}>
            {this.state.sent ? `重新获取${this.state.timeout}S` : `获取验证码`}
          </Text>
        </View>
        {/*登录按钮*/}
        <TouchableOpacity activeOpacity={0.8} onPress={() => this.submit()}>
          <View style={[styles.btn, {backgroundColor: '#d0648f'}]}>
            <Text
              allowFontScaling={false}
              style={{fontSize: px(30), color: '#fff'}}>
              登录
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <View style={{position: 'absolute', bottom: px(80)}}>
          <TouchableOpacity
            ...
            <Image
              source={{uri: require('../images/icon-wechat')}}
              style={{width: px(38), height: px(38), marginRight: px(13)}}
            />
            <Text
              allowFontScaling={false}
              style={{color: '#679d5e', fontSize: px(28)}}>
              微信登录
            </Text>
          </TouchableOpacity>
        </View>
