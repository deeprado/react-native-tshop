          ...
          <View style={styles.expressItem} key={index}>
            <TouchableWithoutFeedback onPress={() => this.props.goDetail}>
              <View style={[styles.skuItemBase, styles.skuItem]}>
                <Image
                  style={styles.skuItemImage}
                  source={{uri: item.prodImg}}
                />
                <View style={styles.skuItemInfo}>
                  <Text allowFontScaling={false} style={styles.skuItemFont}>
                    {item.goodsName}
                  </Text>
                </View>
                <View style={styles.skuItemPrice}>
                  ...
                  <Text
                    ...
                    style={styles.skuItemFontSmall}>
                    x{item.prodQty}
                  </Text>
                  {item.refundQty > 0 && (
                    <Text
                      allowFontScaling={false}
                      style={styles.skuItemFontShow}>
                      已退货:{item.refundQty}
                    </Text>
                  )}
                </View>
    ...
    return (
      <View style={orderStyles.order}>
        <View>
          {/*订单编号/状态*/}
          <TouchableWithoutFeedback
            onPress={() => this.goDetail(order.orderNo)}>
            <View style={orderStyles.orderHeader}>
              <Text allowFontScaling={false} style={orderStyles.orderNo}>
                订单编号 {order.orderNo}{' '}
              </Text>
              <Text allowFontScaling={false} style={orderStyles.orderStatus}>
                {order.orderStatusNm}
              </Text>
            </View>
          </TouchableWithoutFeedback>
          ...
          <View style={orderStyles.orderFooter}>
            <View style={{flexDirection: 'column'}}>
              ...
              {order.orderStatusNm === '待支付' && (
                <Text
                  allowFontScaling={false}
                  style={{fontSize: px(20), color: '#858385'}}>
                  付款剩余时间60:00:00
                </Text>
              )}
            </View>
            {order.orderStatusNm === '待支付' && (
              <View style={{flexDirection: 'row'}}>
                <Text
                  onPress={this.cancel}
                  style={[orderStyles.cBtn, orderStyles.cancelBtn]}
                  allowFontScaling={false}>
                  取消订单
                </Text>
                <Text
                  onPress={this.pay}
                  style={[orderStyles.cBtn, orderStyles.payBtn]}
                  allowFontScaling={false}>
                  去支付
                </Text>
              </View>
            ...
            {order.orderStatusNm === '已完成' && (
              <View style={{flexDirection: 'row'}}>
                <Text
                  style={{color: '#858385', fontSize: px(24)}}
  ...
  render() {
    return (
      <FlatList
        ...
        renderItem={({item}) => (
          <OrderItem order={item} navigation={this.props.navigation} />
        )}
        ListEmptyComponent={
          <Text allowFontScaling={false} style={orderStyles.emptyList}>
            {this.state.empty}
          </Text>
        }
      />
      ...
      for (let index = 0; index < 10; index++) {
        ...
        for (let j = 0; j < 2; j++) {
          expressList.push({
            id: j,
            prodImg:
              'http://img4.daling.com/data/files/mobile/2017/11/30/15120349684184.jpg_300x300.jpg',
  ...
  render() {
    return (
      <View style={{flex: 1}}>
        ...
        <DialogModal ref={dialog => (this.dialog = dialog)} />
      </View>
    );
  }
  renderList(title) {
