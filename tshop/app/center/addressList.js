    ...
    return (
      <View style={styles.addressItem} key={address.id}>
          ...
          <View style={styles.addressInfo}>
            {address.defaultYn === 'Y' && (
              <View style={styles.addressCheck}>
                <Image
                  source={{uri: require('../images/icon-address-check')}}
        ...
        <View style={styles.addressAction}>
          <TouchableWithoutFeedback
            onPress={() => this.props.defaultSelect(address.id)}>
            <View style={styles.radio}>
              {address.defaultYn === 'Y' && (
                <Image
                  source={{uri: require('../images/icon-default-address')}}
                  resizeMode="cover"
              ...
              {address.defaultYn === 'N' && (
                <Image
                  source={{uri: require('../images/icon-default-address-un')}}
                  resizeMode="cover"
                  style={{width: px(34), height: px(34)}}
                />
              )}
          ...
          <TouchableWithoutFeedback
            onPress={() => this.props.goEdit(address.id)}>
            <View style={styles.addressActionBtn}>
              <Image
                style={{width: px(24), height: px(24), marginTop: px(1)}}
                source={{uri: require('../images/icon-address-edit')}}
              />
              <Text allowFontScaling={false} style={styles.addressActionBtnTxt}>
                编辑
              </Text>
            </View>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback
            onPress={() => this.props.delAddress(address.id)}>
            <View style={styles.addressActionBtn}>
              <Image
                style={{width: px(25), height: px(25)}}
                source={{uri: require('../images/icon-address-del')}}
              />
              <Text allowFontScaling={false} style={styles.addressActionBtnTxt}>
                删除
              </Text>
    ...
    return (
      <View style={styles.container}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyList} allowFontScaling={false}>
              暂无收货地址
            </Text>
          }
        />
